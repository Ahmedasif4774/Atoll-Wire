...
function DeadlineBadge({ article }: { article: Article }) {
  ...
  return <span className="deadline-chip">⏳ {formatDaysLeftLabel("dv", getDaysLeft(article.appeal.deadlineDate))}</span>;
}

// All the actual markup/styling for the dv category page lives here, in a
// Client Component, because it uses <style jsx> — which (as of Next 14)
// can't be used directly inside a Server Component. The page.tsx next to
// this file stays a Server Component (it needs to be, for
// generateStaticParams) and just does the data lookup, then hands the
// plain, already-serializable results to this component as props.
      ...
      <main className="wrap">
        ...
        {major && (
          <div className="feature-strip">
            <a className="feature-major" href={`/article/${major.slug}`}>
            ...
            {secondary.length > 0 && (
              ...
              >
                {secondary.map((a) => (
                  <a key={a.slug} className="tile" href={`/article/${a.slug}`}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="content-split">
          {articles.length > 0 ? (
            <div className="news-feed">
              {articles.map((a) => (
                <a key={a.slug} className="feed-item" href={`/article/${a.slug}`}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={a.heroImage.url} alt={a.heroImage.alt} />
                  <DeadlineBadge article={a} />
                  <div className="overlay">
              ...
              <div className="load-more">
                <a href="#">
                  <span className="arrow">▼</span> އިތުރު ޚަބަރު
                </a>
              </div>
            </div>
          ) : (
            <div className="category-empty">
              ...
              <p>މި ބައިގައި އަދި ޚަބަރެއް ނެތް</p>
            </div>
          )}
          ...
          <aside className="popular">
            ...
            <h3>މަޤުބޫލު ޚަބަރު</h3>
            {popular.map((a, idx) => (
              <a key={a.slug} className="pop-item" href={`/article/${a.slug}`}>
